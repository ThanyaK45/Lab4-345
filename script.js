...
const modal = document.getElementById("myModal");
    ...
    openModalButton.addEventListener("click", () => {
      ...
      if (currentNumber730 > 0) {
        selectedProducts.push(`ASRV เสื้อกันหนาวมีฮู้ด Unisex Top Coat : ${currentNumber730} ชิ้น`);
      }
      if (currentNumber21 > 0) {
        selectedProducts.push(`Converse Run Star Hike Lugged- Hi - Foundational Canvas - 166800C - 166800CH0BK : ${currentNumber21} ชิ้น`);
      }
      if (currentNumber55 > 0) {
        selectedProducts.push(`AirPods (3rd generation) : ${currentNumber55} ชิ้น`);
      }
      if (currentNumber50 > 0) {
        selectedProducts.push(`FARUILINE STORE กางเกงคาร์โก้ลําลอง : ${currentNumber50} ชิ้น`);
      }
      ...
      const summ = (currentNumber730 * 273.00) + (currentNumber21 * 3600.00) + (currentNumber55 * 5890.00) + (currentNumber50 * 363.00);
      let final_summ = summ;
      let discountMessage = "";
      let priceBefMessage = "";
